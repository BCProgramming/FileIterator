...
namespace BCFileSearch
{
    public class DebugLogger : TraceListener
    {
        ...
        public static DebugLogger Log = new DebugLogger(Assembly.GetExecutingAssembly().GetName().Name);
        ...
        public override void Write(String LogMessage)
        {
            try
            {
                lock (logStreamLock)
                {
                    ...
                    LogStream.Write(Thread.CurrentThread.ManagedThreadId + "|" + DateTime.Now.ToString("hh:mm:ss") + ">>" + LogMessage);
                    LogStream.Flush();
                }
            }
            catch (Exception exx)
